...
const Lowerer = struct {
    ...
    fn buildConstType(self: *Lowerer, ty: Type.TypeId) Common.LowerError!const_store.ConstType {
        return switch (self.types.get(ty)) {
            ...
            .callable => |variants| return try self.constFuncTypeForCallable(variants),
    ...
    fn constFuncTypeForCallable(self: *Lowerer, variants_span: Type.Span) Common.LowerError!const_store.ConstType {
    ...
    fn lowerBlockInto(self: *Lowerer, target: LIR.LocalId, stmts_span: Lifted.Span(Lifted.StmtId), final_expr: Lifted.ExprId, next: LIR.CFStmtId) Common.LowerError!LIR.CFStmtId {
        return try self.lowerBlockIntoAtType(target, try self.lowerExprTy(final_expr), stmts_span, final_expr, next);
    }
    ...
    fn lowerPatternThenAtType(self: *Lowerer, pat_id: Lifted.PatId, pat_ty: Type.TypeId, source: LIR.LocalId, on_match: LIR.CFStmtId, miss: ?PatternMiss, continuation: LIR.CFStmtId) Common.LowerError!LIR.CFStmtId {
        ...
        return switch (pat_data.data) {
            ...
            .nominal => |inner| try self.lowerPatternThenAtType(inner, self.runtimeBackingType(pat_ty), source, on_match, miss, continuation),
    ...
    fn bindPatternAtType(self: *Lowerer, pat_id: Lifted.PatId, pat_ty: Type.TypeId, source: LIR.LocalId, next: LIR.CFStmtId) Common.LowerError!LIR.CFStmtId {
        ...
        return switch (pat_data.data) {
            ...
            .nominal => |inner| try self.bindPatternAtType(inner, self.runtimeBackingType(pat_ty), source, next),
    ...
    fn layoutsMatch(self: *Lowerer, lhs: layout.Idx, rhs: layout.Idx) bool {
    ...
    fn assignCallableBoundary(
        ...
        for (source_variants, 0..) |source_variant, source_index| {
            const target_index = self.callableVariantIndexBySource(target_variants, source_variant.source);
    ...
    fn assignCaptureRecordBoundary(
        ...
        while (i > 0) {
            i -= 1;
            const source_index = self.captureFieldIndexInFields(source_fields, target_fields[i]);
    ...
    fn assignRecordBoundary(
        ...
        while (i > 0) {
            i -= 1;
            const source_index = self.recordFieldIndexInFields(source_fields, target_fields[i].name);
            current = try self.assignStructFieldBoundary(
                target_locals,
                @intCast(i),
                target_fields[i].ty,
    ...
    fn assignTypedValueIntoStorage(
        ...
        const source_value = if (self.layoutsMatch(source_storage_layout, source_value_layout))
            source_storage
        else
        ...
        if (self.layoutsMatch(target_storage_layout, target_value_layout)) {
    ...
    fn callableVariantIndexBySource(self: *Lowerer, variants: []const Type.FnVariant, source: Common.Symbol) usize {
        _ = self;
    ...
    fn captureFieldIndexInFields(self: *Lowerer, fields: []const Type.CaptureField, needle: Type.CaptureField) usize {
        _ = self;
        for (fields, 0..) |field, index| {
    ...
    fn recordFieldIndexInFields(self: *Lowerer, fields: []const Type.Field, name: Type.names.RecordFieldNameId) usize {
        _ = self;
        for (fields, 0..) |field, index| {
    ...
    fn assignLocal(self: *Lowerer, target: LIR.LocalId, source: LIR.LocalId, next: LIR.CFStmtId) Common.LowerError!LIR.CFStmtId {
        ...
        if (@import("builtin").mode == .Debug) {
            ...
            const target_content = self.result.layouts.getLayout(target_layout);
            const source_content = self.result.layouts.getLayout(source_layout);
            if (target_layout != source_layout and !target_content.eql(source_content)) {
