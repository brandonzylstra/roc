...
- [ ] Replace `BodyContext.lowerType` with a node-returning API for active body
      lowering, and keep a closed `TypeId` API only for code that runs outside
      an active specialization graph.
- [ ] Replace active-body `graph.monoFor` call sites with node refs or with
      node-composition helpers that return `NodeId`.
- [ ] Replace active-body `graphFunctionType` helpers with graph function-node
...
- [ ] Update lambda and closure lowering so expected function types are
      ...
      request is sealed.
- [ ] Update call lowering so caller and callee communicate through explicit
      checked type relations and imported sealed types, but active call result,
      argument, and callee function types remain graph nodes inside the draft.
- [ ] Update generated helper lowering for parser runtime, encode-to runtime,
      ...
      draft store.
- [ ] Keep hosted and imported-cache function records sealed when they are
...
- [ ] Seal deferred procedure-template requests before lowering reserved bodies:
...
- [ ] During sealing, translate draft ids and spans to final shard-local ids and
      spans exactly once.
- [ ] During sealing, update `SpecRecord` identities and the direct
...
- [ ] Delete range-based final-array sealing once no active lowering path writes
...
- [ ] Add an architecture check that rejects new active-body calls to
...
- [ ] Add a cache round-trip test that builds with the draft path, writes a
...
- [ ] Capture a clean-machine benchmark after deterministic counters show the
      expected shape. This must run on an otherwise quiet machine or dedicated
      benchmark host; this worktree is suitable for deterministic counters and
      correctness tests, not timing conclusions.
